...
dnl
dnl In order to account for these discrepances and avoid having to duplicate the
dnl Markdown sources, we create a single m4 template for each unit, using the
dnl macros defined below to make the necessary customisations. The Markdown
